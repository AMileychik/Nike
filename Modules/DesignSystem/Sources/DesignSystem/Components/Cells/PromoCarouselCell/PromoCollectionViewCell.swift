...
public final class PromoCollectionViewCell: UICollectionViewCell {
    ...
    private let backgroundContainerView: ShadowView = {
        ...
        view.configureShadow(color: .lightGray, opacity: 0.2, offset:  CGSize(width: 0, height: 2), radius: 2)
...
extension PromoCollectionViewCell {
    ...
    public func configure(with model: PromoPageViewData) {
        pageControlImageView.image = UIImage(named: model.image ?? "")
        pageControlTitleLabel.text = model.title
        pageControlDescriptionLabel.text = model.subtitle
    }
}
...
extension PromoCollectionViewCell {
    ...
    public func addLeftCornerRadius(to view: UIView, radius: CGFloat) {
